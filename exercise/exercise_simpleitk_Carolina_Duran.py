import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(sys.argv[0]), 'D:/Eigene Dateien/Dokumente/0_MIALab/MIALab/'))  # append the MIALab root directory to Python path
...
def load_image(img_path, is_label_img):
    ...
    img = sitk.ReadImage(img_path, outputPixelType)  # todo: modify here
...
def register_images(img, label_img, atlas_img):
    ...
    registered_img = sitk.Resample(img, atlas_img, transform, interpolator, defaultPixelValue, img.GetPixelIDValue())  # todo: modify here
    ...
    registered_label = sitk.Resample(label_img, atlas_img, transform, interpolator, defaultPixelValue, outputPixelType)  # todo: modify here
...
def preprocess_rescale_numpy(np_img, new_min_val, new_max_val):
    ...
    rescaled_np_img = (new_max_val-new_min_val)*(np_img-min_val) / (max_val-min_val) + new_min_val  # todo: modify here
